...
- **Status:** Superseded by [ADR-0009](0009-inline-intake-and-cron-sweeps.md)
- **Date:** 2026-09-16
...
| Broadcast expiry                            | Needs a timer per open posting                                             |
...
- `step.sleep` handles the multi-hour waits (broadcast expiry, stalled checkout) without polling or a cron table. This is the single biggest reason to adopt a workflow engine rather than a job queue.
...
| **Vercel cron + a status table**                    | Works for coarse polling, but not for per-broadcast timers or step-level retries. Would require a bespoke state machine in the database                                     |
